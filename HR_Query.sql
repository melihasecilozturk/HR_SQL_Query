--1 Select persons who still working (OUTDATE is null) in the company
SELECT
  *
FROM 
  PERSONS
...
--2 Select total women and men who is stil working for each department
SELECT
  DP.DEPARTMENT,
 CASE
...
--3 Select total women and men who is stil working for each department (as column)
SELECT
  D.DEPARTMENT,
...
--4 Select min, max and average salary for 'PLANLAMA ŞEFİ'
SELECT 
  POSITION,
...
--5 Write a query that shows total recruitment male and female for each year
SELECT
...
--6 Write the query that shows how many months each employee has worked.
SELECT 
  NAME_+' '+SURNAME,
  INDATE,
  OUTDATE,
...
--7 Write the query that shows total shortnames
SELECT
...
--8 Write the query that shows departments ordinary salary greater than 5500
SELECT
  DEPARTMENT,
  ROUND(AVG(SALARY), 0) AS AVG_SALARY
FROM 
...
--9 Write the query that captures the average seniority of the departments.
SELECT 
  DEPARTMENT, 
...
--10 Write the query that shows person name, position, manager and manager position for each person
SELECT 
...
FROM 
  PERSONS AS P
  JOIN POSITIONS AS PS ON PS.ID=P.POSITIONID
  JOIN PERSONS AS P2 ON P2.MANAGERID=P.MANAGERID
